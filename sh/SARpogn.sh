...
python $PATHSRC/processogn.py 					>>proc$(date +%y%m%d).log
echo "============="        					>>proc$(date +%y%m%d).log
echo $(date +%H:%M:%S)      					>>proc$(date +%y%m%d).log
echo "============="        					>>proc$(date +%y%m%d).log
python $PATHSRC/buildogndb.py prt 				>>proc$(date +%y%m%d).log
echo "============="        					>>proc$(date +%y%m%d).log
python $PATHSRC/buildogndb.py MYSQL DATA$(date +%y%m%d).log  	>>proc$(date +%y%m%d).log
echo "============="        					>>proc$(date +%y%m%d).log
echo $(date +%H:%M:%S)      					>>proc$(date +%y%m%d).log
echo "============="        					>>proc$(date +%y%m%d).log
python $PATHSRC/analysisrelay.py -n DATA$(date +%y%m%d).log -i 5	>>proc$(date +%y%m%d).log
echo "============="        					>>proc$(date +%y%m%d).log
echo $(date +%H:%M:%S)      					>>proc$(date +%y%m%d).log
echo "============="        					>>proc$(date +%y%m%d).log
sleep 180
/bin/echo '/bin/sh $PATHSRC/sh/SARpogn.sh' | at -M $(calcelestial -n -p sun -m set -q Madrid -H civil) + 15 minutes
...
mv FD*    $dir
mv proc*.log log
/bin/echo '/bin/sh $PATHSRC/sh/SARflight_logger.sh' | at -M $(calcelestial -n -p sun -m rise -q Madrid) + 60 minutes
rm -f tmp/*.IGC
atq 
rm sent
