...
echo "============="        					>>proc$(date +%y%m%d).log
sleep 180
/bin/echo '/bin/sh /home/pi/src/SARpogn.sh' | at -M $(calcelestial -p sun -m set -q Madrid -H civil) + 15 minutes
...
mv FD*    $dir
mv proc*.log log
/bin/echo '/bin/sh /home/pi/src/SARflight_logger.sh' | at -M $(calcelestial -p sun -m rise -q Madrid) + 60 minutes
rm -f tmp/*.IGC
cd 
rm sent
